...
<button type="submit" class="list-group-item" style="background-color:transparent;border:none;color:#2895ED;text-align:left;" name="display_trash"><i class="fas fa-trash-alt" style="color:#2895ED;margin-right:15px;"></i>Trash</button>
 ...
 <div class="col-md-10">
 <div class="card" style="height:800px;width:1280px;margin-left:-20px;">
 ...
 <div class="col-md-4">
 <!--<img src="vr_project_images/Bmail_logo2.png" style="width:150px;height:150px;margin-left:20px;" class="img-responsive rounded-circle"> -->
 <div class="card" style="margin-top:40px;height:450px;">
 <div class="card-header" style="background-color:#d8d8d8;">Trending feedback</div>
 ...
 <?php
  ...
  while($row=mysqli_fetch_array($result))
  {
		  ...
		  <p style='text-align:right;'>-$row[username]<hr>";
  }
  }
  else
  {
 ...
 <input type="text" name="username" style="width:350px;height:40px;">
 </div>
 <div class="col-md-6">
 <label>Bmail Id:</label><br>
 <input type="email" name="mail_id" style="width:350px;height:40px;">
 </div>
 </div>
 
 <div class="row" style="margin-top:30px;margin-left:0px;">
 <label>Feedback & Suggestions</label><br>
 <textarea name="feedback_box" rows="20" style="width:770px;height:200px;overflow-y:scroll;">
 </textarea>
 </div>
 <br><br>
 <button type="submit" name="submit_feedback" class="btn btn-primary" style="text-align:center;width:200px;margin-left:580px;">Submit Feedback</button>
 </form>
 </div>    <!--col-md-8 end----->
...
if(isset($_POST['submit_feedback']))
{
	 ...
	 $query="insert into feedback_table(username,feedback,user_id) values('$username','$feedback','$user_id') ";
	 $result=mysqli_query($con,$query);
	 if($result)
	 {
	 	   echo "<script>alert('Thanks for your feedbackj')</script>";
		   echo "<script>window.open('sidebar_test.php','_self')</script>";
	 }
	 else
	 {
	 	   echo "<script>alert('Feedback not sent properly.Try again later')</script>";
		   echo "<script>window.open('sidebar_test.php','_self')</script>";
	 }
}
 ?>
